...
const OtherBoards = () => {
    ...
    return (
        <div>   
                <div className='mt-10 grid 2xl:grid-cols-5 xl:grid-cols-4 lg:grid-cols-3 px-5 md:grid-cols-2 place-items-center'> {/* grid 2xl:grid-cols-5 xl:grid-cols-4 lg:grid-cols-3 px-10 md:grid-cols-2 */}
                    {Object.keys(otherBoards).map((i, j) => {
                        ...
                        return (
                            <div className='self-center justify-center'>
                                <BoardTile key={j} cover={"data:image/jpg;base64,"+cover} title={otherBoards[i]["title"]} ownerFirstName={otherBoards[i]["owner"].firstName} ownerLastName={otherBoards[i]["owner"].lastName} ownerId={otherBoards[i]["owner"]._id} createdAt={otherBoards[i]["createdAt"]} boardId={otherBoards[i]._id} ownerImage={ownerImage}/>
                            </div>
