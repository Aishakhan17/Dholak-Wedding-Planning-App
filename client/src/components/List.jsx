...
const List = ({listTitle, id, boardId, listChange, errorMessageUpdate}) => {
    ...
    async function createCard(event) {
        ...
        let cardStatus = await axios.post(
            `${process.env.REACT_APP_API_URL}/cards/create-card`, {
                id, cardTitle
            }
        )
        ...
        <div className='mt-5 ml-5 h-fit min-h-content w-64 min-w-48 bg-card bg-opacity-80 rounded-md justify-center'>
            ...
            </div>
            {
                cards.length > 0 
                    ...
                    : <div>
                        <p className='mt-2 text-center text-sm font-bold leading-9 tracking-tight text-white self-center justify-center'>No Cards yet</p>
                    </div>
            }
            <div>
                {
                    !active
                        ?   <div className='w-full flex flex-row justify-evenly p-2'>
                                ...
                                <button className='text-white text-center justify-center self-center' onClick={handleBlur}>Add new card</button>
                            </div>
